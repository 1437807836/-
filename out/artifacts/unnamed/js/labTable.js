let LayUIDataTable = (function() {
  ...
  function checkJquery() {
    if (!$) {
      console.log(
        "未获取jquery对象，请检查是否在调用ConvertDataTable方法之前调用SetJqueryObj进行设置！"
      );
      return false;
    } else return true;
  }
  ...
  function ConvertDataTable(callback) {
    ...
    if (isExistFixColFlag) {
      EachRowDealData(
        trArrFiex,
        dataList,
        LayUIDataTableConfig.isdblClick,
        callback
      );
    }
    return dataList;
  ...
  function IsExistFiexColumn() {
    if (IsExistFiexColumnL() || IsExistFiexColumnR()) return true;
    else return false;
  }
  ...
  function IsExistFiexColumnL() {
    if ($(".layui-table-fixed.layui-table-fixed-l").length > 0) return true;
    else return false;
  }
  ...
  function IsExistFiexColumnR() {
    if ($(".layui-table-fixed.layui-table-fixed-r").length > 0) return true;
    else return false;
  }
      ...
      $.each(tdArrObj, function(index_1, tdObj) {
        ...
        if ($($(tdObj).find("div.layui-form-checkbox")).length > 0) {
          ...
          rowData["checkbox"]["checkbox"] =
            $($(tdObj).find("div.layui-form-checkbox")).length > 0
              ? $($(tdObj).find("div.layui-form-checkbox"))
              : undefined;
          if (IsExistFiexColumn()) {
            if (!fixedCol) fixedCol = GetFiexColumnArr();
            if (IsExistFiexColumnL())
              rowData["fixedColumn_L"] = $(fixedCol[indexFixedColumn + 1]);
            if (IsExistFiexColumnR()) {
              rowData["fixedColumn_R"] = $(
                fixedCol[fixedCol.length / 2 + indexFixedColumn + 1]
              );
            }
            indexFixedColumn++;
          }
